...
/*
    Created by Christopher Batu

    ChatServer class provide a GUI for the user to see
    the various connections that have been established as
    well as the current active user list. This is performed
    with the help of multi threading, The server listens
    for a connection from a client continuously and create
    a new thread to handle the communication with the
    connected socket.
    When the user tries to connect to the server for the
    first time, as client side can only send the userName,
    it is checked whether there is a user with same name in
     the userList maintained in on the server side and if the
     username is unique, an accepted message is sent to
     the client and the client is then only allowed to receive
     messages and become active in the userList.
 */
public class ChatServer extends Application {
     ...
     */
    ObservableList<String> logItems = FXCollections.observableArrayList (logList);
    ...
    public void start(Stage primaryStage) {
        ...
        to the client has been closed properly before closing the
        application.
         */
    ...
    /*
    When this method is called, it makes sure that all the
    ...
    /*
    This thread creates a new serverSocket using the port 8000
    and waits for the user to connect. This is done in a loop so
    that this server will be waiting and creating a new
    connection as the user joins the server.
     */
    private void listen() {
        try {
            ...
            Platform.runLater(() -> logItems.add("MultiThreadServer started at " + new Date()));
    ...
    void sendOnlineStatus(Socket socket,String message) {
        for (Enumeration<DataOutputStream> e = getOutputStreams(); e.hasMoreElements();) {
            DataOutputStream dataOutputStream = e.nextElement();
            try {
    ...
    each socket in different thread.
     */
    class ServerThread extends Thread {
        ...
        public void run() {
            try {
                ...
                while (true) {
                    /*
                    When the user connects to the server for first time, as it
                    can only send userName, the userName is checked against
                    the userList to make sure only one user with the same
                    name exist, and approve message is sent if approved.
                    */
                    if(!userJoined) {
                     ...
                     /*
                    Once it is joined, it can receive the message from the other
            ...
            /*
            Whenever Exception is thrown due to closed socket, this is
            handled properly so that further error do not occur due
            to nonexistent socket. The user is also removed from the
